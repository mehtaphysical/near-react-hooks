...
var _default = exponentialBackoff;
...
var require$$2$1 = /*@__PURE__*/getAugmentedNamespace(lib$1);
...
var NotEnoughBalance = "Sender {{signer_id}} does not have enough balance {{balance}} for operation costing {{cost}}";
var NotEnoughAllowance = "Access Key {account_id}:{public_key} does not have enough balance {{allowance}} for transaction costing {{cost}}";
...
var TriesToStake = "Account {{account_id}} tries to stake {{stake}}, but has staked {{locked}} and only has {{balance}}";
...
var TriesToUnstake = "Account {{account_id}} is not yet staked, but tries to unstake";
...
var DeleteAccountHasRent = "Account {{account_id}} can't be deleted. It has {{balance}}, which is enough to cover the rent";
var DeleteAccountHasEnoughBalance = "Account {{account_id}} can't be deleted. It has {{balance}}, which is enough to cover it's storage";
...
var rpc_error_types = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
...
class CacheError extends errors.TypedError {
}
...
class ActionsValidationError extends errors.TypedError {
}
...
class BalanceMismatchError extends errors.TypedError {
}
...
class ReceiptValidationError extends errors.TypedError {
}
...
const CLASSMAP = __importStar(rpc_error_types);

__exportStar(rpc_error_types, exports);
class ServerTransactionError extends rpc_error_types.ServerError {
}
...
var require$$0 = /*@__PURE__*/getAugmentedNamespace(network);
...
const network = __importStar(require$$0);
...
class Account {
    ...
    async createAndDeployContract(contractId, publicKey, data, amount) {
        ...
        await this.signAndSendTransaction(contractId, [transaction.createAccount(), transaction.transfer(amount), transaction.addKey(key_pair.PublicKey.from(publicKey), accessKey), transaction.deployContract(data)]);
    ...
    async sendMoney(receiverId, amount) {
        return this.signAndSendTransaction(receiverId, [transaction.transfer(amount)]);
    }
    /**
    ...
    async createAccount(newAccountId, publicKey, amount) {
        const accessKey = transaction.fullAccessKey();
        return this.signAndSendTransaction(newAccountId, [transaction.createAccount(), transaction.transfer(amount), transaction.addKey(key_pair.PublicKey.from(publicKey), accessKey)]);
    }
    /**
    ...
    async deleteAccount(beneficiaryId) {
        return this.signAndSendTransaction(this.accountId, [transaction.deleteAccount(beneficiaryId)]);
    }
    /**
     * @param data The compiled contract code
     * @returns {Promise<FinalExecutionOutcome>}
    ...
    async deployContract(data) {
        return this.signAndSendTransaction(this.accountId, [transaction.deployContract(data)]);
    }
    /**
     * @param contractId NEAR account where the contract is deployed
    ...
    async functionCall(contractId, methodName, args, gas, amount) {
        ...
        return this.signAndSendTransaction(contractId, [transaction.functionCall(methodName, args, gas || DEFAULT_FUNC_CALL_GAS, amount)]);
    }
    /**
    ...
    async addKey(publicKey, contractId, methodNames, amount) {
        ...
        return this.signAndSendTransaction(this.accountId, [transaction.addKey(key_pair.PublicKey.from(publicKey), accessKey)]);
    }
    /**
    ...
    async deleteKey(publicKey) {
        return this.signAndSendTransaction(this.accountId, [transaction.deleteKey(key_pair.PublicKey.from(publicKey))]);
    }
    /**
    ...
    async stake(publicKey, amount) {
        return this.signAndSendTransaction(this.accountId, [transaction.stake(amount, key_pair.PublicKey.from(publicKey))]);
    }
    validateArgs(args) {
    ...
    async viewFunction(contractId, methodName, args, { parse = parseJsonFromRawResponse } = {}) {
        args = args || {};
        this.validateArgs(args);
        const result = await this.connection.provider.query(`call/${contractId}/${methodName}`, lib$2.baseEncode(JSON.stringify(args)));
        if (result.logs) {
            this.printLogs(contractId, result.logs);
        }
...
class Account2FA extends AccountMultisig {
    ...
    async disable(contractBytes) {
        ...
        const actions = [
            ...
            ...lak2fak.map(({ public_key }) => transaction.deleteKey(public_key)),
            ...lak2fak.map(({ public_key }) => transaction.addKey(public_key, null)),
            transaction.deployContract(contractBytes),
        ];
...
const getContract = (account) => {
    return new contract.Contract(account, account.accountId, {
...
class ConnectedWalletAccount extends account.Account {
    ...
    async signAndSendTransaction(receiverId, actions) {
        ...
        await this.walletConnection.requestSignTransactions([transaction$1], window.location.href);
    ...
    if (config.keyPath && config.deps && config.deps.keyStore) {
        try {
            ...
            if (accountKeyFile[0]) {
                ...
                config.deps.keyStore = new key_stores.MergeKeyStore([config.deps.keyStore, keyPathStore]);
 ...
 * @ignore
 */
var NearContext = React.createContext({});
/**
...
var useNear = function () {
    var near = useContext(NearContext).near;
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return near;
};
var useNearWallet = function () {
    var wallet = useContext(NearContext).wallet;
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return wallet;
};
/**
...
export { NearEnvironment, NearProvider, useNear, useNearContract, useNearWallet };
